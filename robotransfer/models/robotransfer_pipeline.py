...
from .guider import GuiderNet


class RoboTransferPipeline(StableVideoDiffusionPipeline):
    model_cpu_offload_seq = (
        "image_encoder->unet->vae->normal_guider_net->depth_guider_net"
    )
    ...
    def __init__(
        ...
        depth_guider_net: GuiderNet,
        normal_guider_net: GuiderNet,
        ...
        self.register_modules(
            ...
            normal_guider_net=normal_guider_net,
        )
    ...
    def check_guider_image(self, guider_images: torch.Tensor, batch_size: int):
        if guider_images.ndim == 4:
            if batch_size != 1:
                raise ValueError(
                    f"Got image input with batch size of {batch_size}, but depth_guider_images with ndim 4."
                )
        elif guider_images.ndim != 5:
            depth_guider_ndim = guider_images.ndim
            raise ValueError(
                f"Got depth_guider_images with ndim of {depth_guider_ndim}"
            )
    ...
    def __call__(
        ...
        self.check_guider_image(depth_guider_images, batch_size)
        self.check_guider_image(normal_guider_images, batch_size)

        if depth_guider_images.ndim == 4:
        ...
        num_channels_latents = (
            ...
            - image_latents.size(2)
            - self.depth_guider_net.config.out_channels
            - self.normal_guider_net.config.out_channels
        )
        ...
        normal_guider_images = rearrange(
            normal_guider_images, "b f c h w -> (b f) c h w"
        )
        normal_condition_latents = self.normal_guider_net(normal_guider_images)
        normal_condition_latents = rearrange(
            normal_condition_latents, "(b f) c h w -> b f c h w", b=batch_size
        )

        depth_condition_latents = (
            torch.cat([depth_condition_latents] * 2)
            if self.do_classifier_free_guidance
            else depth_condition_latents
        )
        normal_condition_latents = (
            torch.cat([normal_condition_latents] * 2)
            if self.do_classifier_free_guidance
            else normal_condition_latents
        )
